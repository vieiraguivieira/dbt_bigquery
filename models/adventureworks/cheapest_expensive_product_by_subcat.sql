select 
    ...
    ,first_value(prod.listprice) over (partition by prod.SubCategoryID order by prod.listprice asc) as leastexpensiveprice
    ,first_value(prod.productname) over (partition by prod.SubCategoryID order by prod.listprice desc) as mostexpensive
    ,first_value(prod.listprice) over (partition by prod.SubCategoryID order by prod.listprice desc) as mostexpensiveprice
